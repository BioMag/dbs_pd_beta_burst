...
##### LOAD CONTROL DATA
with open(path_to_procdat + 'controls_all.pkl', 'rb') as file:  # Python 3: open(..., 'rb')
...
# the (downsampled) sampling frequency should be identical for controls & patients
if sfreq_p!=sfreq_p2:
...
BURST_bin_off=[]
BURST_bin_on=[]
for h in range(0, len(BI_all)): # len(BI_all) is 2 (old and new patients)
...
def post_burst_windowing(data_burst_info, data_burst_bin, lim, lim2, sfreq, window):
    ...
    for h in range(0, len(data_burst_info)):
        ...
        # Probability of a burst given a burst
        prob_bgb = list()
        ...
        # Probability of a burst given a long burst (prob_bglb)
        prob_bglb = list()
        ...
        # Probability of a burst given a short burst (prob_bglb)
        prob_bgsb = list()
...
ftitle=(path_to_figs + 'Figure_6.png')          
fig.savefig(ftitle)  