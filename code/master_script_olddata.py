...
[array([3.00e+00, 4.46e+03, 2.00e+00, 1.00e+00]),       #### Pat number, patient id, condition (2=ON, 3=OFF), hand used (1=left)
 array([  1., 180.]),                                   #### EC condition, beginning and end
 array([180., 456., 459., 480.]),                       #### EO condition, beginning and end (pairs)
 array([494., 552.]),                                   #### MOT condition, beginning and end (pairs)
 array([574., 631.]),
 array([654., 712.]),
 array([734., 791.]),
 array([814., 874.])]
...
timing_info=path_to_rawdat + 'MEG_times_old_patients.pkl' 
...
output_file = 'old_patients.pkl'
output_file2 = 'old_patients_psd_spectra.pkl'
...
for g in range(0, len(pat_list)):
    txtfiles = []
    for file in glob.glob(path_trunc + 'raw_data/spontti_old_data/%s*standard.fif' % pat_list[g]):
